...
async function compressWav(arrayBuffer, preset) {
  ...
  output.set(
    new Uint8Array(writeWavHeader(dataLength, channels, sampleRate, bitDepth)),
    0,
  );
...
export default function App() {
  ...
  const addFiles = useCallback((newFiles) => {
    const wavFiles = Array.from(newFiles).filter((f) =>
  ...
  const downloadFile = (item) =>
    triggerDownload(
      item.compressedBlob,
      item.name.replace(/\.wav$/i, "_compressed.wav"),
    );
  ...
  const p = QUALITY_PRESETS[preset];
  const estReduction = Math.max(
    0,
    Math.round(
      (1 - (p.sampleRate * p.channels * p.bitDepth) / (44100 * 2 * 16)) * 100,
    ),
  );
      ...
      <div className="container">
        ...
        >
          ...
          <div className="dropzone-sub">
            Multiple files supported · 100% local
          </div>
        </div>
                      ...
                      <div className="file-meta">
                        ...
                        {item.compressedSize && (
                          <>
                            ...
                            <span
                              className={`size-after ${bigger ? "bigger" : ""}`}
                            >
                            ...
                            {bigger && (
                              <span className="hint">try a lower preset</span>
                            )}
                          </>
                        )}
                        {item.status === "error" && (
        ...
        <div className="footer">
          All processing is local — nothing leaves your browser. WAV output is
          uncompressed PCM at a lower sample rate / bit depth. Files that were
          already low-quality may not shrink with higher presets.
        </div>
      </div>
    </div>
  );
